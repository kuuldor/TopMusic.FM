...
/**
 * Builds the notification for TopMusic's service. Jelly Bean and higher uses the
...
public class NotificationHelper {
     ...
     * Notification ID
     */
    private static final int topmusic_MUSIC_SERVICE = 1;
    ...
    public void buildNotification(final String albumName, final String artistName,
            final String trackName, final String albumId, final Bitmap albumArt) {
        ...
        if (TopMusicUtils.hasHoneycomb()) {
            ...
            mService.startForeground(topmusic_MUSIC_SERVICE, mNotification);
        } else {
    ...
    public void goToIdleState(final boolean isPlaying) {
        ...
        if (TopMusicUtils.hasHoneycomb() && mNotificationTemplate != null) {
        ...
        if (TopMusicUtils.hasJellyBean() && mExpandedView != null && TopMusicUtils.hasJellyBean()) {
            mExpandedView.setImageViewResource(R.id.notification_expanded_base_play,
                    isPlaying ? R.drawable.btn_playback_play : R.drawable.btn_playback_pause);
        }
        try {
            mNotificationManager.notify(topmusic_MUSIC_SERVICE, mNotification);
    ...
    private PendingIntent getPendingIntent() {
        return PendingIntent.getActivity(mService, 0, new Intent("the.topmusic.AUDIO_PLAYER")
