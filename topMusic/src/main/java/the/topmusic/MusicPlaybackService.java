...
import the.topmusic.appwidgets.AppWidgetLarge;
import the.topmusic.appwidgets.AppWidgetLargeAlternate;
import the.topmusic.appwidgets.AppWidgetSmall;
import the.topmusic.appwidgets.RecentWidgetProvider;
...
public class MusicPlaybackService extends Service {
     ...
     */
    public static final String PLAYSTATE_CHANGED = "the.topmusic.playstatechanged";

    /**
     * Indicates the meta data has changed in some way, like a track change
     */
    public static final String META_CHANGED = "the.topmusic.metachanged";

    /**
     * Indicates the queue has been updated
     */
    public static final String QUEUE_CHANGED = "the.topmusic.queuechanged";

    /**
     * Indicates the repeat mode chaned
     */
    public static final String REPEATMODE_CHANGED = "the.topmusic.repeatmodechanged";

    /**
     * Indicates the shuffle mode chaned
     */
    public static final String SHUFFLEMODE_CHANGED = "the.topmusic.shufflemodechanged";

    /**
     * Called to indicate a general service commmand. Used in
     * {@link MediaButtonIntentReceiver}
     */
    public static final String SERVICECMD = "the.topmusic.musicservicecommand";

    /**
     * Called to go toggle between pausing and playing the music
     */
    public static final String TOGGLEPAUSE_ACTION = "the.topmusic.togglepause";

    /**
     * Called to go to pause the playback
     */
    public static final String PAUSE_ACTION = "the.topmusic.pause";

    /**
     * Called to go to stop the playback
     */
    public static final String STOP_ACTION = "the.topmusic.stop";

    /**
     * Called to go to the previous track
     */
    public static final String PREVIOUS_ACTION = "the.topmusic.previous";

    /**
     * Called to go to the next track
     */
    public static final String NEXT_ACTION = "the.topmusic.next";

    /**
     * Called to change the repeat mode
     */
    public static final String REPEAT_ACTION = "the.topmusic.repeat";

    /**
     * Called to change the shuffle mode
     */
    public static final String SHUFFLE_ACTION = "the.topmusic.shuffle";

    /**
     * Called to kill the notification while TopMusic is in the foreground
     */
    public static final String KILL_FOREGROUND = "the.topmusic.killforeground";

    /**
     * Used to easily notify a list that it should refresh. i.e. A playlist
     * changes
     */
    public static final String REFRESH = "the.topmusic.refresh";

    /**
     * Called to build the notification while TopMusic is in the background
     */
    public static final String START_BACKGROUND = "the.topmusic.startbackground";

    /**
     * Called to update the remote control client
     */
    public static final String UPDATE_LOCKSCREEN = "the.topmusic.updatelockscreen";
    ...
    private static final int ARTISTCOLIDX = 1;
    private static final int ALBUMCOLIDX = 2;
    private static final int NAMECOLIDX = 3;
    private static final int DATACOLIDX = 4;
    private static final int TYPECOLIDX = 5;
    private static final int ALBUMIDCOLIDX = 6;
    private static final int ARTISTIDCOLIDX = 7;
    /**
    ...
    /**
     * Returns true if the TopMusic is sent to the background, false otherwise
     */
    ...
    public static boolean isOnlineSong(Song song) {
        return song != null && song.mURL != null && !song.mURL.isEmpty();
    }
    ...
    /**
     * Builds the notification for TopMusic
     */
    public void buildNotification() {
        if (mBuildNotification || TopMusicUtils.isApplicationSentToBackground(this)) {
            try {
     ...
     */
    private Cursor getCursorForId(final String id) {
    ...
    private int getNextPosition(final boolean force) {
        if (!force && mRepeatMode == REPEAT_CURRENT) {
            ...
            for (Integer aMHistory : mHistory) {
                final int idx = aMHistory.intValue();
    ...
    private void setNextTrack() {
        ...
        if (mNextPlayPos >= 0 && mPlayList != null) {
            ...
            if (isOnlineSong(song)) {
                mPlayer.setNextDataSource(song.mURL);

            } else {
                mPlayer.setNextDataSource(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/" + song.mSongId);
            }
        }
    }
    ...
    private boolean makeAutoShuffleList() {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                ...
                final String albumID = cursor.getString(2);
                final String album = cursor.getString(3);
                final String artistID = cursor.getString(4);
                final String artist = cursor.getString(5);
                final String duration = cursor.getString(6);
    ...
    private void notifyChange(final String what) {
        ...
        if (what.equals(META_CHANGED)) {
            ...
            if (mFavoritesCache.getSongId(getAudioId()) != null) {
                mFavoritesCache.addSong(getCurrentSong());
            }
    ...
    private void saveQueue(final boolean full) {
        ...
        if (full) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                q.append(Base64.encodeToString(ParcelableUtils.marshall(song), Base64.DEFAULT));
                q.append(";");
            }
    ...
    private void reloadQueue() {
        ...
        if (qlen > 1) {
            ...
            int plen = items.length;
            ensurePlayListCapacity(plen);
            for (int i = 0; i < plen; i++) {
                byte[] data = Base64.decode(items[i], Base64.DEFAULT);
                mPlayList[i] = ParcelableUtils.unmarshall(data, Song.CREATOR);
            }
            mPlayListLen = plen;
    ...
    public int removeTrack(final String id) {
        ...
        synchronized (this) {
            for (int i = 0; i < mPlayListLen; i++) {
                if (mPlayList[i].mSongId.equals(id)) {
    ...
    private String getArtworThumbnailkURL() {
        synchronized (this) {
            Song song = getCurrentSong();
            if (song == null) {
                return null;
            }
            return song.mThumbnail;
        }
    }
    ...
    public boolean isFavorite() {
        if (mFavoritesCache != null) {
            synchronized (this) {
                final String id = mFavoritesCache.getSongId(getAudioId());
                return id != null;
            }
        }
        return false;
    ...
    public void open(final Song[] list, final int position) {
        synchronized (this) {
            try {
                ...
                mHistory.clear();
                openCurrentAndNext();
                if (oldId == null || !oldId.equals(getAudioId())) {
                    notifyChange(META_CHANGED);
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void toggleFavorite() {
        if (mFavoritesCache != null) {
            synchronized (this) {
                mFavoritesCache.toggleSong(getCurrentSong());
            }
        }
    }
    ...
    public Bitmap getAlbumArt() {
        ...
        if (!isOnlineSong(song)) {
            ...
            bitmap = mImageFetcher.getCachedBitmap(song.mThumbnail);
        }

        if (bitmap != null) {
            //Copy it or it may crash due to recycled bitmap
    ...
    private static final class MultiPlayer implements MediaPlayer.OnErrorListener,
        ...
        private boolean setDataSourceImpl(final MediaPlayer player, final String path) {
            try {
                ...
                if (path.startsWith("content://") || MusicPlaybackService.isNetResource(path)) {
    ...
    private static final class ServiceStub extends ITopMusicService.Stub {
        ...
        @Override
        public String getArtworkURL() throws RemoteException {
            return mService.get().getArtworkURL();
        }

        @Override
        public String getArtworkThumbnailURL() throws RemoteException {
            return mService.get().getArtworThumbnailkURL();
        }
