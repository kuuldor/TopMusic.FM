...
import the.topmusic.lyrics.LyricsProvider;
import the.topmusic.lyrics.LyricsProviderFactory;
import the.topmusic.lyrics.OfflineLyricsProvider;
...
public class LyricsFragment extends SherlockFragment {
    ...
    public void fetchLyrics(final boolean force) {
        if (isAdded()) {
            TopMusicUtils.execute(false, new FetchLyrics(), force);
        }
    }
    ...
    private final class FetchLyrics extends AsyncTask<Boolean, Void, String> {
        ...
        protected String doInBackground(final Boolean... force) {
            ...
            if (lyrics == null && TopMusicUtils.isOnline(getSherlockActivity())) {
                mTryOnline = true;
                provider = LyricsProviderFactory.getMainOnlineProvider();
                lyrics = provider.getLyrics(mArtist, mSong);
                if (lyrics == null) {
                    provider = LyricsProviderFactory.getSecondOnlineProvider();
                    lyrics = provider.getLyrics(mArtist, mSong);
                }
            }
            return lyrics;
