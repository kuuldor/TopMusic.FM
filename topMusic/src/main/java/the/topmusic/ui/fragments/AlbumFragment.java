...
import the.topmusic.adapters.AlbumAdapter;
import the.topmusic.loaders.AlbumLoader;
...
import static the.topmusic.utils.PreferenceUtils.ALBUM_LAYOUT;
...
public class AlbumFragment extends SherlockFragment implements LoaderCallbacks<List<Album>>,
        ...
        if (item.getGroupId() == GROUP_ID) {
            switch (item.getItemId()) {
                ...
                case FragmentMenuItems.DELETE:
                    ...
                    String[] list = MusicUtils.songListToIDList(mAlbumList);

                    DeleteDialog.newInstance(album, list, album + Config.ALBUM_ART_SUFFIX)
    ...
    private int getItemPositionByAlbum() {
        final String albumName = MusicUtils.getCurrentAlbumId();
    ...
    private void initGridView() {
        ...
        if (TopMusicUtils.isLandscape(getSherlockActivity())) {
            if (isDetailedLayout()) {
