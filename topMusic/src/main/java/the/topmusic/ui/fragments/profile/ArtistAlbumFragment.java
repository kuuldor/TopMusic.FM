...
import the.topmusic.adapters.ArtistAlbumAdapter;
import the.topmusic.loaders.ArtistAlbumLoader;
...
import the.topmusic.widgets.VerticalScrollListener.ScrollableHeader;
...
public class ArtistAlbumFragment extends SherlockFragment implements LoaderCallbacks<List<Album>>,
     ...
     */
    private static final int GROUP_ID = FragmentIndex.ARTISTALBUM_FRAGMENT_IDX;
        ...
        if (item.getGroupId() == GROUP_ID) {
            switch (item.getItemId()) {
                ...
                case FragmentMenuItems.DELETE:
                    int len = mAlbumList.length;
                    String[] list = new String[len];
                    for (int i = 0; i < len; i++) {
                        list[i] = mAlbumList[i].mSongId;
                    }
                    DeleteDialog.newInstance(mAlbum.mAlbumName, list, null).show(
    ...
    public Loader<List<Album>> onCreateLoader(final int id, final Bundle args) {
        return new ArtistAlbumLoader(getSherlockActivity(), args.getString(Config.ID));
    }
