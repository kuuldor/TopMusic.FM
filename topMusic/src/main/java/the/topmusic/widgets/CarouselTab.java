...
import the.topmusic.utils.BitmapUtils;
...
public class CarouselTab extends FrameLayoutWithOverlay {
    ...
    public CarouselTab(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        mFetcher = TopMusicUtils.getImageFetcher((SherlockFragmentActivity) context);
    }
    ...
    public void setAlbumPhotoWithURL(final SherlockFragmentActivity context, final String url) {
        if (!TextUtils.isEmpty(url)) {
            mAlbumArt.setVisibility(View.VISIBLE);
            mFetcher.loadImageFromURL(url, mAlbumArt);
        } else {
            setDefault(context);
        }
    }
    ...
    public void setURLPhoto(final SherlockFragmentActivity context,
                            final String url) {
        if (!TextUtils.isEmpty(url)) {
            mFetcher.loadImageFromURL(url, mPhoto);
        } else {
            setDefault(context);
        }
    }
