...
import the.topmusic.lastfm.Album;
import the.topmusic.lastfm.Artist;
import the.topmusic.lastfm.Image;
import the.topmusic.lastfm.ImageSize;
import the.topmusic.lastfm.PaginatedResult;
...
public class ImageFetcher extends ImageWorker {
    ...
    public static File downloadBitmapToFile(final Context context, final String urlString,
        ...
        final File cacheDir = DiskCacheUtils.getDiskCacheDir(uniqueName);
                ...
                if (!TextUtils.isEmpty(artistName)) {
                    if (PreferenceUtils.getInstace(mContext).downloadMissingArtistImages()) {
                        ...
                        if (paginatedResult != null) {
                            for (Image temp : paginatedResult.pageResults) {
    ...
    public void loadCurrentArtwork(final ImageView imageView) {
        String imageURL = MusicUtils.getArtworkURL();
    ...
    public void loadCurrentArtworkThumbnail(final ImageView imageView) {
        String imageURL = MusicUtils.getArtworkThumbnailURL();
        if (imageURL != null && !imageURL.isEmpty()) {
            loadImageFromURL(imageURL, imageView);
        } else {
            loadImage(MusicUtils.getAlbumName() + Config.ALBUM_ART_SUFFIX, MusicUtils.getArtistName(),
